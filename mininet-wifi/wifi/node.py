...
from mininet-original.log import info, error, warn, debug
from mininet-original.util import (quietRun, errRun, isShellBuiltin)
from mininet-original.node import Node
from mininet-original.moduledeps import moduleDeps, pathCheck, TUN
from mininet-original.link import Link, Intf, OVSIntf
from mininet-wifi.wifi.link import TCWirelessLink, TCLinkWirelessAP,\
    Association, wirelessLink, adhoc, mesh
from mininet-wifi.wifi.wmediumdConnector import WmediumdServerConn, \
    WmediumdPosition, WmediumdTXPower, WmediumdGain, WmediumdHeight, \
    wmediumd_mode, WmediumdConstants
from mininet-wifi.wifi.propagationModels import distanceByPropagationModel, \
    powerForRangeByPropagationModel, propagationModel
from mininet-wifi.wifi.util import moveIntf
from mininet-original.utils.private_folder_manager import PrivateFolderManager
...
class Node_wifi(Node):
    ...
    def setRange(self, value, intf=None):
        "Set Signal Range"
        from mininet-wifi.wifi.plot import plot2d
        wlan = 0
        if intf is None:
            wlan = self.params['wlan'].index(intf)
    ...
    def updateGraph(self):
        "Update the Graph"
        from mininet-wifi.wifi.plot import plot2d, plot3d
        cls = plot2d
        if plot3d.is3d:
            cls = plot3d
        if cls.fig_exists():
